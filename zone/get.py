...
def get_zones_by_heating(reverse=1):
    ...
    for zone in zones:
        ...
        if Post.objects.filter(zone_id=zone.zone_id, post_status=1).count():
            zone.zone_heating += int(Post.objects.filter(zone_id=zone.zone_id, post_status=1).count() * 900)
        if Post.objects.filter(zone_id=zone.zone_id, post_status=1).aggregate(
                Sum('post_view')).get('post_view__sum'):
            zone.zone_heating += Post.objects.filter(zone_id=zone.zone_id, post_status=1).aggregate(
                Sum('post_view')).get('post_view__sum') * 50
        if Post.objects.filter(zone_id=zone.zone_id, post_status=1).aggregate(
                Sum('post_like')).get('post_like__sum'):
            zone.zone_heating += Post.objects.filter(zone_id=zone.zone_id, post_status=1).aggregate(
                Sum('post_like')).get('post_like__sum') * 150
        if Review.objects.filter(post_id__in=zone.post_set.values_list('post_id', flat=True)).count():
            zone.zone_heating += int(Review.objects.filter(
                post_id__in=zone.post_set.values_list('post_id', flat=True)).count() * 200)
        if Collection.objects.filter(post_id__in=zone.post_set.values_list('post_id', flat=True)).count():
            zone.zone_heating += int(
                Collection.objects.filter(post_id__in=zone.post_set.values_list('post_id', flat=True)).count() * 300)
    if reverse == 1:
